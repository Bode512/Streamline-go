<script lang="ts">
  ...
  async function loadQR() { qrURL = await QRDataURL() }
  ...
  async function uploadLocal() {
    ...
    for (const file of localFiles) {
      let offset = 0
      uploadStatus = `Subiendo ${file.name}...`
      while (offset < file.size) {
        const end = Math.min(offset + chunkSize, file.size)
        const response = await fetch(api(`/upload?file=${encodeURIComponent(file.name)}&deviceId=desktop&deviceInfo=Desktop&offset=${offset}&total=${file.size}`), { method: 'POST', headers: { 'Content-Type': 'application/octet-stream' }, body: file.slice(offset, end) })
        if (!response.ok) throw new Error(`No se pudo subir ${file.name}`)
        const result = await response.json()
        offset = Number(result.offset ?? end)
      }
    }
    uploadStatus = localFiles.length ? 'Vídeos enviados a la cola' : ''
    localFiles = []
    await refresh()
  }
...
<main>
  ...
  <section class="hero">
    ...
    <div class="connect-box"><label for="server">Servidor Streamline</label><div class="server-input"><input id="server" bind:value={server} /><button title="Conectar" onclick={() => { refresh(); loadQR(); connectEvents() }}>→</button></div>{#if error}<p class="error">{error}</p>{:else}<p class="hint">Instancia local o cualquier equipo en tu red.</p>{/if}</div>
  </section>

  <section class="local-upload panel"><div><p class="eyebrow">LOCAL IMPORT</p><h2>Convierte desde este ordenador</h2><p>Selecciona uno o varios vídeos. Se copiarán al Core, se convertirán y el original irá a la Papelera al terminar correctamente.</p></div><div class="upload-controls"><input id="local-files" type="file" accept="video/*" multiple onchange={(event) => localFiles = Array.from((event.currentTarget as HTMLInputElement).files ?? [])} /><button class="select-button" onclick={uploadLocal} disabled={!localFiles.length}>{localFiles.length ? `Convertir ${localFiles.length} vídeo${localFiles.length > 1 ? 's' : ''}` : 'Seleccionar vídeos'}</button>{#if uploadStatus}<small>{uploadStatus}</small>{/if}</div></section>
  ...
  <section class="content-grid">
    <div class="panel jobs"><div class="panel-head"><div><p class="eyebrow">RECENT ACTIVITY</p><h2>Trabajos</h2></div><button class="refresh" title="Actualizar" onclick={refresh}>↻</button></div>
      {#if history.length === 0}<div class="empty">Todavía no hay vídeos en la biblioteca.</div>{:else}<div class="job-list">{#each history.slice(0, 7) as item}<div class="job"><div class="preview">{#if item.status === 'ready'}<video src={api(`/preview?file=${encodeURIComponent(item.filename)}`)} preload="metadata" muted onloadedmetadata={(event) => durations[item.filename] = formatDuration((event.currentTarget as HTMLVideoElement).duration)}></video><span class="play">▶</span>{:else}<span class="format">{item.filename.split('.').pop()?.toUpperCase().slice(0, 3)}</span>{/if}</div><div class="file"><strong>{item.filename}</strong><small>{item.deviceInfo || item.deviceId} · {formatDate(item.uploadTime)}</small><div class="meta">{formatBytes(item.currentSize || item.originalSize)} {#if item.currentSize && item.originalSize && item.currentSize !== item.originalSize} · original {formatBytes(item.originalSize)}{/if} {#if durations[item.filename]} · {durations[item.filename]}{/if}</div></div><span class="status" class:ready={item.status === 'ready'} class:failed={item.status === 'failed'}>{item.status}</span>{#if item.status === 'processing'}<button class="action" title="Cancelar" onclick={() => jobAction('cancel', item.filename)}>×</button>{:else if item.status === 'failed'}<button class="action" title="Reintentar" onclick={() => jobAction('retry', item.filename)}>↻</button>{/if}</div>{/each}</div>{/if}
    </div>
    <aside class="panel share"><p class="eyebrow">MOBILE DROP</p><h2>Envía desde tu móvil</h2><p>Escanea el código para abrir el panel de subida en tu red local.</p>{#if qrURL}<img class="qr" src={qrURL} alt="Código QR de Streamline" />{:else}<div class="qr qr-empty">Generando QR...</div>{/if}<div class="share-url">{server}/</div></aside>
  </section>
...
<style>
  ...
  .local-upload { display: flex; justify-content: space-between; gap: 28px; align-items: center; margin-bottom: 12px; } .local-upload p { max-width: 620px; } .upload-controls { display: flex; gap: 10px; align-items: center; flex-wrap: wrap; justify-content: end; } .upload-controls input { max-width: 220px; padding: 8px; font: 11px Arial, sans-serif; } .select-button { width: auto; padding: 12px 16px; white-space: nowrap; } .upload-controls small { width: 100%; color: #8ac28b; font: 11px Arial, sans-serif; text-align: right; }
  @media (max-width: 760px) { main { padding: 20px; } .hero, .content-grid, .local-upload { display: block; } .connect-box { width: auto; margin-top: 32px; } .stats { grid-template-columns: repeat(2, 1fr); margin-top: 12px; } .share { margin-top: 12px; } .upload-controls { justify-content: start; margin-top: 16px; } footer { display: block; line-height: 2; } }
</style>
