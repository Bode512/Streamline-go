<script lang="ts">
  ...
  type Item = { filename: string; deviceId: string; deviceInfo: string; status: string; uploadTime: number }
  ...
  onMount(() => {
    ServerURL().then((value) => { server = value; refresh(); loadQR(); connectEvents() }).catch(() => { refresh(); loadQR(); connectEvents() })
